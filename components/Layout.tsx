...
export const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  return (
    <div className="min-h-screen bg-alphabag-black text-alphabag-text font-sans selection:bg-alphabag-yellow selection:text-alphabag-black">
      <Header toggleSidebar={() => setIsSidebarOpen(!isSidebarOpen)} isSidebarOpen={isSidebarOpen} />
      
      <Sidebar isOpen={isSidebarOpen} onClose={() => setIsSidebarOpen(false)} />

      <main className="pt-16 md:pl-64 min-h-screen transition-all duration-300 pb-20 md:pb-6">
      ...
      <div className="fixed bottom-20 md:bottom-6 right-6 z-[100] flex flex-col space-y-3 pointer-events-none">
        {toasts.map(toast => (
            <div 
                key={toast.id}
                className="pointer-events-auto bg-alphabag-dark border border-alphabag-gray px-4 py-3 rounded-xl shadow-2xl flex items-center space-x-3 min-w-[280px] animate-slide-in overflow-hidden"
            >
                <div className={`
