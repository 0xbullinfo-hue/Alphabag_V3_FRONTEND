...
export const AlphaPasses: React.FC = () => {
  ...
  const incrementQuantity = () => { if (quantity < NFT_CONFIG.MAX_MINT_PER_TX) setQuantity(p => p + 1); };
              ...
              <div className="mt-4 space-y-2">
                ...
                <div className="flex items-center gap-3">
                  <div className="flex items-center bg-alphabag-black border border-alphabag-gray rounded-md p-1">
                    ...
                    <button type="button" onClick={incrementQuantity} disabled={quantity >= NFT_CONFIG.MAX_MINT_PER_TX || isApproving || isMintingTx} className="w-9 h-9 rounded bg-alphabag-gray hover:bg-alphabag-gray/80 text-alphabag-text font-bold flex items-center justify-center transition-all disabled:opacity-40">+</button>
                  </div>
                  <div className="flex items-center gap-1.5 flex-1">
                    {[1, 2, 5, 10].map((preset) => (
                      <button key={preset} type="button" onClick={() => setQuantity(preset)} disabled={isApproving || isMintingTx}
